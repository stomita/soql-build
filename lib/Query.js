...
var Query = function (_QueryElement) {
  ...
  function Query(config) {
    ...
    var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(Query).call(this));
  ...
  _createClass(Query, [{
    ...
    value: function toSOQL() {
      return ['SELECT', this.fields.map(function (field) {
        return field.toSOQL();
      }).join(', '), 'FROM', this.table].concat(_toConsumableArray(this.condition ? ['WHERE', this.condition.toSOQL()] : []), _toConsumableArray(this.sortInfo && this.sortInfo.length > 0 ? ['ORDER BY', this.sortInfo.map(function (s) {
        return s.toSOQL();
